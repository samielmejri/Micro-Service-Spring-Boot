...
import tn.esprit.user.services.Implementations.ActivityServices;
...
public class ActivityController {
    ...
    @Autowired
    private UserHamzaRepo userHamzaRepo;
...
public ResponseEntity<?> saveActivity(@RequestBody Activity activity) {
    try {
        ...
        String message = "Your activity '" + activity.getActivityName() + "' has been successfully created/updated!";
        //activityServices.sendSms(activity.getMobile(), message);
        activityServices.sendSms("+21650307811", message);
        return ResponseEntity.ok(activity.get_id());
    } catch (Exception e) {
        return ResponseEntity.badRequest().body("Failed to save activity: " + e.getMessage());
    }
}
